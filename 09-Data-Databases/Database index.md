...
wikipedia_categories: ["Database index techniques", "Database management systems", "Databases"]
related: ["[[Composite index (database)]]", "[[Concurrency control]]", "[[Data masking]]", "[[Data store]]", "[[Database design]]", "[[Datasource]]", "[[Document-oriented database]]", "[[Foreign key]]", "[[Imieliński–Lipski algebra]]", "[[Materialized view]]"]
---
...
When the wildcard characters are used on both sides of the search word as %wikipedia.org%, the index available on this field is not used. Rather only a sequential search is performed, which takes ⁠
...
⁠ time.
To mitigate this, a full-text indexing approach can be used. For example, text can be split into trigrams (3-character chunks) and stored in a GIN (Generalized inverted index) to reduce complexity of lookup to about 
...
This can be done in SQL with CREATE INDEX my_index ON my_table (id) INCLUDE (name);.
...
== See also ==
Index locking
Inverted index – Type of database index
