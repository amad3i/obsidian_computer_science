...
wikipedia_categories: ["Cryptographic protocols", "Hypertext Transfer Protocol", "Internet properties established in 1994", "Secure communication", "Transport Layer Security", "URI schemes"]
related: ["[[CCMP (cryptography)]]", "[[OCSP stapling]]", "[[Online Certificate Status Protocol]]", "[[Security Protocols Open Repository]]", "[[Server Name Indication]]", "[[Temporal Key Integrity Protocol]]", "[[Transport Layer Security]]", "[[X.1035]]", "[[Application-Layer Protocol Negotiation]]", "[[AS1 (networking)]]"]
---
...
Forcing a web browser to load only HTTPS content has been supported in Firefox starting in version 83. Starting in version 94, Google Chrome is able to "always use secure connections" if toggled in the browser's settings. Prior to version 117, Google Chrome displayed a lock icon in the address bar, which has since been replaced by a "tune" icon. Many users believe that a lock icon implies a website is safe, ignoring other security concerns.
