...
wikipedia_categories: ["Distributed computing problems", "Fault-tolerant computer systems"]
related: ["[[Byzantine fault]]", "[[Automatic vectorization]]", "[[Availability zone]]", "[[Big memory]]", "[[Brooks–Iyengar algorithm]]", "[[Computer cluster]]", "[[Data lineage]]", "[[Data redundancy]]", "[[Database transaction schedule]]", "[[Distributed concurrency control]]"]
---
...
There is a t-resilient anonymous synchronous protocol which solves the Byzantine Generals problem, if ⁠t/n⁠ < ⁠1/3⁠ and the Weak Byzantine Generals case where t is the number of failures and n is the number of processes.
For systems with n processors, of which f are Byzantine, it has been shown that there exists no algorithm that solves the consensus problem for n ≤ 3f in the oral-messages model. The proof is constructed by first showing the impossibility for the three-node case n = 3 and using this result to argue about partitions of processors. In the written-messages model there are protocols that can tolerate n = f + 1.
...
Other protocols, such as Cerberus, have been developed to apply Byzantine fault-tolerant consensus to sharded distributed ledgers and have been the subject of academic analysis.
An example of a polynomial time binary consensus protocol that tolerates Byzantine failures is the Phase King algorithm by Garay and Berman. The algorithm solves consensus in a synchronous message passing model with n processes and up to f failures, provided n > 4 f.
