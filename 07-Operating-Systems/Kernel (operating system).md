...
wikipedia_categories: ["Operating system kernels", "Operating systems"]
...
Using a high-level interface, such as a Video BIOS
Using a lower-level device driver (file system drivers using disk drivers)
...
Using an interrupt. This method is available on most hardware, and is therefore very common.
...
Using a special system call instruction. This technique requires special hardware support that common architectures (notably, x86) may lack; for those architectures, interrupts or call gates are used. System call instructions have been added to recent models of x86 processors, however, and some operating systems for PCs make use of them when available.
...
The principle of separation of mechanism and policy is the substantial difference between the two main philosophies of microkernels and monolithic kernels. Here a mechanism is the support that allows the implementation of many different policies, while a policy is a particular "mode of operation". Example:

Mechanism: User login attempts are routed to an authorization server
Policy: Authorization server requires a password that is verified against stored passwords in a database
...
In a minimal microkernel just some very basic policies are included, and its mechanisms allow what is running on top of the kernel (the remaining part of the operating system and the other applications) to decide which policies to adopt (such as for memory management, high level process scheduling, file system management, etc.). A monolithic kernel instead tends to include many policies, therefore restricting the rest of the system to rely on them.
...
In CP-40 and CP-67, IBM further abstracted the supervisor state from the hardware, resulting in a hypervisor that enabled full virtualization, i.e. the capacity to run multiple operating systems on the same machine totally independent of each other. 
In the 1970s, IBM updated CP/CMS for the System/370 under the name Virtual Machine Facility/370 (VM/370).
...
Kernel Comparison: Linux (2.6.22) versus Windows (Vista) at the Wayback Machine (archived 2008-09-14)
