...
wikipedia_categories: ["Combinatorial optimization", "Game artificial intelligence", "Graph algorithms", "Graph distance", "Greedy algorithms", "Heuristic algorithms", "Routing algorithms", "Search algorithms"]
related: ["[[Dijkstra's algorithm]]", "[[B-]]", "[[Iterative deepening A-]]", "[[SMA-]]", "[[Alpha–beta pruning]]", "[[Combinatorial search]]", "[[Contraction hierarchies]]", "[[Floyd–Warshall algorithm]]", "[[Johnson's algorithm]]", "[[Minimax]]"]
---
...
⁠ is the true distance from the start to ⁠
...
⁠ (the start is one such vertex).  The next node in ⁠
