...
wikipedia_categories: ["B-tree", "Computer-related introductions in 1971", "Database index techniques"]
related: ["[[BitFunnel]]", "[[Bitmap index]]", "[[Composite index (database)]]", "[[Dancing tree]]", "[[Database index]]", "[[Hierarchical navigable small world]]", "[[Inverted index]]", "[[Z-order curve]]"]
---
...
== Definition ==
According to Knuth's definition, a B-tree of order m is a tree that satisfies the following properties:
...
Bayer and McCreight (1972), Comer (1979), and others define the order of a B-tree as the minimum number of keys in a non-root node. Folk and Zoellick point out that terminology is ambiguous because the maximum number of keys is unclear. An order 3 B-tree might hold a maximum of 6 keys or a maximum of 7 keys. Knuth (1998) avoids the problem by defining the order to be the maximum number of children (which is one more than the maximum number of keys).
...
For simplicity, most authors assume there is a fixed number of keys that fit in a node. The basic assumption is that the key size and node size are both fixed. In practice, variable-length keys may be employed.
...
If ki exists in any node in a B-tree, then ki-1 exists in that node where 
...
Usually, the number of keys is chosen to vary between d and 
...
, where d is the minimum number of keys, and 
...
If an internal node has 
 ...
 keys; joining the neighbor would add d keys plus one more key brought down from the neighbor's parent. The result is an entirely full node of 
...
Sorting and searching algorithms can be characterized by the number of comparison operations that must be performed using order notation. A binary search of a sorted table with N records, for example, can be done in roughly ⌈ log2 N ⌉ comparisons. If the table had 1,000,000 records, then a specific record could be located with at most 20 comparisons: ⌈ log2 (1,000,000) ⌉ = 20.
...
If underflow happens, rebalance the tree as described in the section "Rebalancing after deletion" below.
...
Move all elements from the right node to the left node (the left node now has the maximum number of elements, and the right node is empty)
...
Some early operating systems, and some highly specialized ones, required the application to allocate the maximum size of a file when the file was created. The file can then be allocated as contiguous disk blocks. In that case, to convert the file block address 
...
. In contrast, an (a,b)-tree allows the minimum number of children for an internal node to be set arbitrarily low. In an (a,b)-tree, each internal node has between a and b children, for some preset values of a and b.
...
B-Trees: Balanced Tree Data Structures Archived 2010-03-05 at the Wayback Machine
...
Counted B-Trees
B-Tree .Net, a modern, virtualized RAM & Disk implementation Archived 2016-03-04 at the Wayback Machine
Bulk loading

Shetty, Soumya B. (2010). A user configurable implementation of B-trees (Thesis). Iowa State University.
Kaldırım, Semih (28 April 2015). "File Organization, ISAM, B+ Tree and Bulk Loading" (PDF). Ankara, Turkey: Bilkent University. pp. 4–6. Archived (PDF) from the original on 2022-10-09.
"ECS 165B: Database System Implementation: Lecture 6" (PDF). University of California, Davis. 9 April 2010. p. 23. Archived (PDF) from the original on 2022-10-09.
