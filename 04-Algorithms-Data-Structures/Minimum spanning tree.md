...
wikipedia_categories: ["Polynomial-time problems", "Spanning tree"]
related: ["[[3SUM]]", "[[Bellman–Ford algorithm]]", "[[Borůvka's algorithm]]", "[[Christofides algorithm]]", "[[Collision problem]]", "[[Emptiness problem]]", "[[Floyd–Warshall algorithm]]", "[[K shortest path routing]]", "[[Kruskal's algorithm]]", "[[Longest common subsequence]]"]
---
...
The fractional MST problem can be solved in polynomial time using the ellipsoid method. However, if we add a requirement that f(e) must be half-integer (that is, f(e) must be in {0, 1/2, 1}), then the problem becomes NP-hard, since it includes as a special case the Hamiltonian cycle problem: in an 
...
The ultrametric backbone is the smallest possible distance backbone of positive weighted directed and undirected graphs.  The ultrametric backbone is the union of minimum spanning forests in undirected (positive weight) graphs and provides a generalization of minimum spanning trees to directed graphs that, unlike minimum equivalent graphs and minimum spanning arborescences, preserves all max-min shortest paths and De Morgan's law consistency.
