...
wikipedia_categories: ["1953 in computing", "Hash-based data structures"]
related: ["[[Concurrent hash table]]", "[[Distributed hash table]]", "[[InterPlanetary File System]]", "[[Rainbow table]]", "[[Tor (network)]]"]
---
...
The performance of open addressing may be slower compared to separate chaining since the probe sequence increases when the load factor 
 ...
 approaches 1. The probing results in an infinite loop if the load factor reaches 1, in the case of a completely filled table. The average cost of linear probing depends on the hash function's ability to distribute the elements uniformly throughout the table to avoid runs, since formation of runs would result in increased search time.
