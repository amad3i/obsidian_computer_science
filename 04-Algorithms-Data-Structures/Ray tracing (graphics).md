...
wikipedia_categories: ["3D computer graphics", "Computer graphics", "Geometrical optics", "Global illumination algorithms", "Ray tracing (graphics)", "Shading", "Virtual reality"]
related: ["[[Per-pixel lighting]]", "[[Screen space directional occlusion]]", "[[Distributed ray tracing]]", "[[Horizon mapping]]", "[[Kinematic chain]]", "[[Path tracing]]", "[[Rasterisation]]", "[[Spatiotemporal reservoir resampling]]", "[[3D computer graphics]]", "[[Computers & Graphics]]"]
---
...
Ray tracing–based rendering techniques that sample light over a domain typically generate multiple rays and often rely on denoising to reduce the resulting noise.
...
Pre-calculations: let's find and normalize vector 
...
This is often used when objects cannot be easily represented by explicit surfaces (such as triangles), for example when rendering clouds or 3-D medical scans.
...
Ray tracing a torus Archived February 12, 2019, at the Wayback Machine
