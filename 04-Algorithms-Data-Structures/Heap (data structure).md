...
wikipedia_categories: ["Heaps (data structures)"]
related: ["[[Heapsort]]"]
---
...
⁠, and its parent is at index ⌊(i−1)/2⌋ in an array starting from index ⁠
...
⁠, and ⌊i/2⌋, respectively, in an array starting from ⁠
...
⁠. This simple indexing scheme makes it efficient to walk "up" or "down" the tree.
...
Selection algorithms: A heap allows access to the smallest or largest element in constant time. The k-smallest element in a given heap can be found in O(k) time. Finding the (lower and upper if necessary) median using the heap can be done in O(n) time, which results from setting k ≈ n/2 and thus requires a time linear in n.  
